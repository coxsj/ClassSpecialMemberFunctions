...
// Can move to rule of 4 1/2 if use by-value assignment operator and a swap non-member (friend) function

// Rationale
// =======
// DESTRUCTOR is responsible for freeing the resources correctly.
// If you write a destructor, you probably should write a copy constructor AND a copy assignment operator.
// COPY CONSTRUCTOR is responsible for duplicating resources to avoid double-frees. Double-frees occur when
// a new object is created via shallow copying a pointer which of course points back to the resource owned by
// the original object. Shallow copying is what the default copy constructor does.
...
// If no copy assignment operator has been defined, it uses the default which applies shallow copying.
// So, copy assignment has potentially the same problem as copying.
...
// the COPY ASSIGNMENT constructor is at risk of deleting the data in the source object.
// So, COPY ASSIGNEMENT OPERATOR should use COPY CONSTRUCTOR to make a local copy,
// then use a swap member function - COPY AND SWAP idiom.
// RAII - RESOURCE ACQUISITION IS INITIALIZATION
// RAII slogan is about initialization, but it's really about CLEANUP!
...
// To make an object NON-COPYABLE, explicitly set copy constructor and copy assignment operator to "= delete".
// DEFAULT SPECIAL MEMBER FUNCTIONS
// Explicitly setting special member functions to "= default" make code selff documenting. 
// i.e. "= default"  says "I considered if the default special member functions were acceptable, and they are,
...
// SPECAL CASE: const lvalue reference will bind to rvalue!!
// void f(int&);				f(i);	// Ok			f(42);	// Error
// foid g(int&&);				g(i);	// Error		g(42)	// Ok
// void h(const int&);		h(i)	// Ok			h(42)	// Ok
// Combine this with overload resolution (and knowing that rvalues wont be missed) -> MOVE CONSTRUCTOR
...
// MOVE CONSTRUCTOR doesnt need to bother with the slow copying operations, just steal the guts of the rvalue. 
// In practice, use std::exchange e.g. ptr_ = std::exchange(rhs.ptr_, nullptr); // rhs.ptr_ goes to ptr_, nullptr goes to rhs.ptr_
// RULEOF FIVE
// ===========
// The need for performance adds the MOVE CONSTRUCTOR and MOVE ASSIGNMENT OPERATOR.
...
// However this similarity provides an opportunity to set both assignment operators up as call by value, 
// leaving the copy decision up to the caller. This leads to the rule of four and a half
// RULE OF FOUR AND A HALF
// =======================
// 4 1/2: Destructor, copy constructor, move constructor, by-value assignment operator, and (1/2) a non-member 
// swap function, and ideally a member version too. (If you don't, std::swap gets called which in turn calls move 
// assignment which calls swap...)
// Note: By-value assignment operator requires passing a copy of the object on the stack to the operator. 
...
public:
	...
	// The rules only apply to functions implicit in managing resources.
	...
	// DESTRUCTOR
	// ============
	// Destructor is needed to free the resource (but not if you use smart pointers to manage the resource).
	...
	// Compiler generated default copy constructor just does a shallow copy, even for pointers, file handles etc.
	...
	// fpr the copy). When both source and copy objects are eventually destroyed, their destructors each free the 
	// same memory - DOUBLE FREE == BAD.
	...
	// ==============================
	// The copy & move assignment operators are similar. 
	// We get both by creating a by-value assignment operator.
	// This is a less common implementation of copy & move assignment.
	// This approach transfers move semantics responsibility to the caller.
	// It also ensures the moved or copied item is passed on the stack i.e. not great for large objects
	...
	// This decouples any aliasing relationship between *this and rhs
	...
	// The friend function is in the body of our class i.e. in its namespace
	...
	void push_back(int newValue) 
	{
		...
		if (size_ > last_ + 1) {
			...
			std::cout << "push_back fail due to full\n";
	}
